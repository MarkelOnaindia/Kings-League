...
public class TCAL_XML_Resultado {
    ...
    public static void insertar(CAL_XML_Resultado xml) throws Exception
    {
        ...
        PreparedStatement ps = BaseDatos.getCon().prepareStatement("insert into CAL_XML_Resultado values (?,?,?)");
    ...
    {
        BaseDatos.abrirConexion();
        PreparedStatement ps = BaseDatos.getCon().prepareStatement("delete from CAL_XML_Resultado where IdXML = ?");
        ps.setInt(1, xml.getIdXML());
    ...
    {
        BaseDatos.abrirConexion();
        PreparedStatement ps = BaseDatos.getCon().prepareStatement("update CAL_XML_Resultado set Result = ?, Tipo = ? where IdXML = ?");
    ...
    {
        BaseDatos.abrirConexion();
        PreparedStatement ps = BaseDatos.getCon().prepareStatement("select * from CAL_XML_Resultado where IdXML = ?");
        ps.setInt(1, xml.getIdXML());
